...
from commonClass.C_except import comperaExcept
...
class http(object):
    ...
    def sendHttp(URL,params,header,timeout):
        ...
        if params=='':
            ...
            lgtest.info("请求地址："+URL)
            lgtest.info("请求参数："+params)
    ...
    def testHttp(caseName, db, sql, url, params, Except):
        ...
        Except = Except
        header = {'Content-Type': 'application/json'}
        lgtest.info('用例名称：' + caseName)
        respone = http.sendHttp(url, params, header, 15)
        lgtest.info("实际结果为："+respone.text)
        lgtest.info("预期结果为："+Except)
        return respone
